...
export default function BrochureRequests() {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error("Failed to fetch brochure requests");
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this brochure request?")) return;
    try {
      const res = await fetch(`/api/brochure-requests?id=${id}`, { method: "DELETE" });
      if (!res.ok) throw new Error("Failed to delete brochure request");
  ...
  return (
    <div className="p-8">
      <h1 className="text-2xl font-bold mb-6" style={{ color: '#495565' }}>Brochure Requests</h1>
            ...
            <tbody style={{ color: '#495565' }}>
              {requests.map((req, idx) => (
                <tr key={req._id} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="py-2 px-4 align-middle">{req.propertyTitle}</td>
                  <td className="py-2 px-4 align-middle">{new Date(req.createdAt).toLocaleString()}</td>
